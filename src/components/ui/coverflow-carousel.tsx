...
export interface CoverflowCarouselProps {
  ...
  /** Whether the carousel auto-scrolls continuously without interaction. */
  autoPlay?: boolean;
  /** Speed of auto-scroll in cards moved per second. */
...
export function CoverflowCarousel({
  ...
  /** Animation frame ref for drag release / settle momentum */
  const rafRef = React.useRef<number | null>(null);
  /** Animation frame ref for continuous smooth autoplay */
  const rafAutoRef = React.useRef<number | null>(null);
  ...
  // Autoplay start / stop management
  const startAutoPlay = React.useCallback(() => {
    ...
    // Cancel settle if running so they never fight
    ...
    const step = (time: number) => {
      ...
      // Avoid giant leap if tab was backgrounded
      if (delta > 0 && delta < 0.25) {
        const increment = autoPlaySpeed * delta;
        posRef.current = loop
          ? posRef.current + increment
          : Math.min(count - 1, posRef.current + increment);
        targetRef.current = posRef.current;
        paint();

        const newIndex = indexAt(posRef.current);
        setSelected((prev) => (prev !== newIndex ? newIndex : prev));
      }

      rafAutoRef.current = requestAnimationFrame(step);
    };

    rafAutoRef.current = requestAnimationFrame(step);
  }, [autoPlay, autoPlaySpeed, count, indexAt, loop, paint]);

  const stopAutoPlay = React.useCallback(() => {
    if (rafAutoRef.current !== null) {
  ...
  const settle = React.useCallback(
    (target: number) => {
      // Pause continuous autoplay while settle is animating to snapped card
      if (rafAutoRef.current !== null) {
        cancelAnimationFrame(rafAutoRef.current);
        rafAutoRef.current = null;
      }
      ...
      const step = () => {
        ...
        // ponytail: exponential ease-out, not a spring.
    ...
    [indexAt, paint, scheduleAutoPlayResume],
  );
